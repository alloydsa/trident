...
When a user adds the `tracked` label to a GitHub issue, the workflows automatically:
...
5. **NEW**: Syncs comments from GitHub to Jira automatically
6. **NEW**: Syncs status updates (closed/reopened) to Jira
...
- **`workflows/sync-issue-to-jira.yml`**: Single workflow that handles all GitHub-Jira syncing
  - Creates Jira ticket when "tracked" label is added
  - Syncs comments from GitHub to Jira (one-way)
  - Syncs issue status (closed/reopened) to Jira
- **`scripts/create_jira_ticket.py`**: Unified Python script with three modes:
  - Default: Create Jira ticket
  - `--sync-comment`: Sync comment to Jira
  - `--sync-status`: Update Jira status
